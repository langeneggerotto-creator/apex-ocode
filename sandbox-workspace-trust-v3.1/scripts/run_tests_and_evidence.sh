#!/usr/bin/env bash
# Runs the full Bite 1 gate: spec gate, clean install proof, adversarial + functional
# test suite, and rollback proof — then writes a single evidence manifest capturing
# exact commands and outcomes. Every field in the manifest reflects a command that
# was actually executed in this run, not an assumed or aspirational result.
set -uo pipefail
...
if [ "$GATE_EXIT" -ne 0 ] || [ "$INSTALL_EXIT" -ne 0 ] || [ "$TEST_EXIT" -ne 0 ] || [ "$ROLLBACK_EXIT" -ne 0 ] || [ "$MANIFEST_EXIT" -ne 0 ]; then
    echo "BITE 1 GATES: FAIL"
    exit 1
fi
echo "BITE 1 GATES: PASS"
