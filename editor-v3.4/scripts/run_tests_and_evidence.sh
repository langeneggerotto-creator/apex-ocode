#!/usr/bin/env bash
# Runs the full Bite 4 gate: spec gate, Monaco vendoring, backend + Playwright
# end-to-end tests, a representative screenshot, and a rollback proof — then
# writes a single evidence manifest capturing exact commands and outcomes.
set -uo pipefail
...
echo "== 1/5 specification gate =="
( cd "$REPO_ROOT" && python3 scripts/ocode_spec_gate.py )
GATE_EXIT=$?

echo "== 2/5 vendoring Monaco =="
bash "$BITE_DIR/scripts/vendor_monaco.sh"

echo "== 3/5 clean-environment install proof =="
INSTALL_LOG="$EVIDENCE_DIR/.install_log.txt"
bash "$BITE_DIR/scripts/install_clean_env.sh" > "$INSTALL_LOG" 2>&1
INSTALL_EXIT=$?
cat "$INSTALL_LOG"

echo "== 4/5 test suite (backend + Playwright e2e) =="
...
SCREENSHOT_WS="$(mktemp -d)/ocode-editor-evidence-ws"
mkdir -p "$SCREENSHOT_WS"
cat > "$SCREENSHOT_WS/example.py" <<'PYEOF'
def greet(name):
    return f"hello, {name}"
PYEOF
...
if [ "$GATE_EXIT" -ne 0 ] || [ "$INSTALL_EXIT" -ne 0 ] || [ "$TEST_EXIT" -ne 0 ] || [ "$ROLLBACK_EXIT" -ne 0 ] || [ "$MANIFEST_EXIT" -ne 0 ]; then
    echo "BITE 4 GATES: FAIL"
    exit 1
fi
echo "BITE 4 GATES: PASS"
