#!/usr/bin/env bash
# Installs ocode_tasks (and its local monorepo dependency, ocode_sandbox) into a
...
VENV_DIR="$(mktemp -d)/ocode-tasks-install-proof"
...
echo "== import proof =="
"$VENV_DIR/bin/python3" -c "import ocode_tasks; print('import OK:', ocode_tasks.__file__)"

echo "== CLI proof =="
"$VENV_DIR/bin/python3" -m ocode_tasks --help >/dev/null
echo "CLI OK"
