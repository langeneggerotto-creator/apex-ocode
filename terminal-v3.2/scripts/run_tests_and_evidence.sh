#!/usr/bin/env bash
# Runs the full Bite 2 gate: spec gate, clean install proof (Bite 1 dependency +
# Bite 2), adversarial + functional test suite, and rollback proof — then writes a
...
if [ "$GATE_EXIT" -ne 0 ] || [ "$INSTALL_EXIT" -ne 0 ] || [ "$TEST_EXIT" -ne 0 ] || [ "$ROLLBACK_EXIT" -ne 0 ] || [ "$MANIFEST_EXIT" -ne 0 ]; then
    echo "BITE 2 GATES: FAIL"
    exit 1
fi
echo "BITE 2 GATES: PASS"
